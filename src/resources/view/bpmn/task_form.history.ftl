		...
		<div id="history" class="accordion-body collapse<#if !historicTaskInstances?? || historicTaskInstances?size lt 4> in</#if>">
			<div class="accordion-inner">
				<table class="table">
					<thead>
					<tr>
						<th>${getText('taskName')}</th>
						<th style="width:100px;">${getText('assignee')}</th>
						<th style="width:130px;">${getText('startTime')}</th>
						<th style="width:130px;">${getText('claimTime')}</th>
						<th style="width:130px;">${getText('endTime')}</th>
					</tr>
					</thead>
					<tbody>
					<tr>
						<td>${getText('startProcessInstance')}</td>
